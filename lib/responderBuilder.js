...
class ResponderBuilder extends events_1.EventEmitter {
    ...
    subscribe() {
        return __awaiter(this, void 0, void 0, function* () {
            this.subscriptionId = yield this.worker.subscribe(this._key, this.eventEmitter, true);
        });
    }
    unsubscribe() {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    createEventEmitter() {
        ...
        this.eventEmitter.on(events_2.Events.MESSAGE, (message, content) => {
            let route = message.properties.type || events_2.Events.ROUTE_DEFAULT;
            if (!this.listenerCount(route)) {
                route = events_2.Events.ROUTE_NOT_FOUND;
            }
            this.emit(route, message, content, (res) => this.worker.respond(res, message));
        });
        this.eventEmitter.on(events_2.Events.ERROR, (error, message) => {
            this.emit(events_2.Events.ERROR, error, message, (res) => this.worker.respond(res, message));
        });
    }
}
